...
import { likeRecipe, saveRecipe } from "@/app/actions/user-action"

const RecipeCard = ({ el, session }) => {
    ...
    useEffect(() => {
        //get likes
        const likes = el.userLikes.some(el => el.userId === userId)
        if (likes) {
            setIsLiked(true)
        }

        //get saves
        const saves = el.userRecipes.some(el => el.userId === userId)
        if (saves) {
            setIsSaved(true)
        }
    ...
    return (
        <div dir="rtl" className="relative w-[280px] mx-10 my-6 hover:scale-[1.02] duration-300">
            ...
            <div className="relative w-[280px] h-[350px]">
                <Link href={(el.is_paying === 'T-Telecom' &&  notPremium) ? '/premium/login' : `/recipe/${el.id}`}>
                    <Image
                        src={el.imgPath}
                        alt="recipe image"
                        fill
            ...
            <div className="p-2">
                <div className="flex items-start justify-between text-darkblue mt-2">
                    <p className="text-[22px] w-64 overflow-hidden line-clamp-2 leading-7">{el.title}</p>
                    {/* <Bookmark
                        onClick={handleSave}
                ...
                <div className="flex items-center likeColor mt-0.5">
                    ...
                    <div className="flex items-center mr-8">
                        ...
                        />
                        <span>{formatNumber(el.likes + el.userLikes.length)}</span>
                    </div>
                </div>
