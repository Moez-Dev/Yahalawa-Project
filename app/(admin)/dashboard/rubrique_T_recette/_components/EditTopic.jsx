...
import { deleteSection, updateTopic } from "@/app/actions/topic-action"


const EditTopic = ({ id, title, section, recipes }) => {
    
    const { pending } = useFormStatus()

    const [selectedOption, setSelectedOption] = useState(section ? section.map(item => ({ value: `${item.recipeId}|${item.idi}`, label: item.idi })) : []);

    const handleCategorie = async (option) => {

        const deletedRecipe = section.find(cat => !option.includes(cat));

        if(deletedRecipe) {
            deleteSection(deletedRecipe.id)
        }
